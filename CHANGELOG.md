...
- **`courier_name` / `courier_phone`** columns on `orders` (DECISION_LOG'd — stays inline on Order until Phase 10/12 graduates a `deliveries` table).
- **Migration `d8499a5e7876 — create_payments_courier_columns`** (prev `ff2951f68321`). Hand-edited; round-trips clean.
- **`OrderLifecycleService`** (`app/domain/orders/lifecycle.py`) — single dispatcher for every admin status transition.
    - `ALLOWED_TRANSITIONS: dict[(from, to), TransitionRule]` is the **single source of truth** for the state machine: `(pending→confirmed)`, `(confirmed→preparing)`, `(preparing→ready_for_pickup|out_for_delivery)`, `(ready_for_pickup→out_for_delivery)`, `(ready_for_pickup→delivered)`, `(out_for_delivery→delivered)`, `(any pre-delivered → cancelled)`, `(delivered → refunded)`. Each rule carries `allowed_roles`, `requires_reason`, `side_effects: tuple[Effect, ...]`.
    - **Side-effect hooks** run in-transaction via the dispatcher: `_release_reservations` (cancel pre-dispatch), `_convert_reservation_to_sold` (mark-ready / dispatch), `_restock_for_cancelled_dispatch` (refused-at-door cancel from `out_for_delivery`), `_record_courier` (dispatch), `_create_refund_payment_row` (refund — COD just flips status; card writes a `Payment(is_refund=True, status='pending')` row for the Phase 10 webhook to settle).
    - Per-branch RBAC: non-super_admin actors restricted to their own `branch_id`; pharmacist forbidden from refunds (`refund` rule's `allowed_roles=('super_admin','branch_manager')`).
    - Cancel reason validated against `CANCEL_REASONS = {customer_request, out_of_stock, payment_failed, fraud_suspected, other}` (PRODUCT §9.5).
    - Every transition writes one `order_status_history` row + one `admin_audit_log` row in the same transaction.
    - Public methods: `confirm`, `start_preparing`, `mark_ready_for_pickup`, `mark_out_for_delivery(courier_name, courier_phone)`, `mark_delivered`, `cancel_by_admin(reason)`, `refund(amount, reason)`, `swap_batch(order_id, item_id, new_batch_id)`.
- **`InventoryService.restock_for_cancelled_dispatch(order_id, actor)`** — pairs every prior `sold` movement on the order with a `received` movement that returns stock to the **same** original batch (PRODUCT §9.1 refused-at-door rule — the same medicine returns to the same batch_number / expiry, never a new batch). Updates `bp.total_quantity` accordingly. Deliberately a no-op for `delivered → refunded` (medicine isn't restocked once delivered).
- **Admin schemas** (`app/domain/orders/admin_schemas.py`): `AdminOrderListItem`, `AdminOrderDetail` (full eager view incl. courier + items + history), `CancelByAdminRequest(reason: Literal[…], notes)`, `DispatchRequest(courier_name, courier_phone)`, `RefundRequest(amount, reason)`, `SwapBatchRequest(new_batch_id)`, `AuditLogEntry`.
- **`ReportService`** (`app/domain/reports/services.py`):
    - `sales_report(branch_id, from_dt, to_dt, top_n)` — single SQL with conditional sums; revenue / units / order_count / AOV / cancelled_count / refunded_count. Cancelled + refunded **excluded from revenue** but counted separately (DECISION_LOG'd).
    - `top_products(branch_id, from_dt, to_dt, limit)` — grouped by `oi.product_id` over non-cancelled orders, ordered by units DESC then revenue DESC.
    - 5-minute Redis TTL cache keyed by `(branch_id, from_dt, to_dt[, limit])` (BACKEND §18.4).
    - `stream_csv(rows, columns, bom=True)` — yields utf-8 bytes with BOM for Excel/Cyrillic compatibility; per-row generator so the route can wrap it in a `StreamingResponse`.
    - Helpers `_to_decimal` / `_load_uuid` (mirrors `GUID.process_result_value` since raw `text()` queries bypass the `TypeDecorator`).
- **3 admin route modules** under `/api/admin/v1`:
    - `orders.py` — `GET /orders` (filters: status, branch_id, payment_method, from/to date, q on order_number/recipient_phone/recipient_name; non-super admins auto-scoped to own branch); `GET /orders/{id}`; `POST /orders/{id}/confirm`; `…/start-preparing`; `…/mark-ready`; `…/dispatch` `{courier_name, courier_phone}`; `…/mark-delivered`; `…/cancel` `{reason, notes}`; `…/refund` `{amount, reason}` (Idempotency-Key REQUIRED — 400 `idempotency_key_required` if missing; idempotency scope = `f"refund:{actor.id}"`); `…/items/{item_id}/swap-batch` `{new_batch_id}`.
    - `reports.py` — `GET /reports/sales?branch_id=…&from_dt=…&to_dt=…&top_n=…&format=json|csv`; `GET /reports/top-products?branch_id=…&from_dt=…&to_dt=…&limit=…&format=json|csv`. CSV variant streams via `ReportService.stream_csv`.
    - `audit.py` — `GET /audit?actor=&entity_type=&entity_id=&action=&from_dt=&to_dt=&page=&page_size=` paginated viewer over `admin_audit_log`.
- **DI factories** (`app/api/deps.py`): `get_order_lifecycle_service`, `get_report_service`. Re-uses Phase 6 inventory + audit factories.
- **32 new tests (428 total)**:
    - Unit: `tests/unit/test_order_lifecycle.py` (14 — full state-matrix matrix; cancel from `pending` releases reservations; cancel from `out_for_delivery` triggers `restock_for_cancelled_dispatch`; mark_ready converts reservation → sold; cannot cancel from `delivered`; cancel without reason → 400 `reason_required`; cancel with bogus reason → 400 `invalid_cancel_reason`; pharmacist cannot refund (403 `forbidden_role`); branch_manager from branch A can't act on branch B order (403 `forbidden_branch`); refund on card-paid order writes a pending `Payment(is_refund=True)` row + flips status to `refunded`; refund on COD order just flips status (no Payment row); `swap_batch` picks new batch + un-reserves old + re-reserves new; `swap_batch` rejects a batch belonging to a different product).
    - Unit: `tests/unit/test_reports_service.py` (4 — sales summary aggregates; top_products desc order; cancelled excluded from revenue but counted; `stream_csv` emits BOM + header + rows).
    - Integration: `tests/integration/test_admin_audit_coverage.py` (5 — every Phase 5 catalog mutation, Phase 6 inventory mutation, Phase 9 lifecycle confirm writes exactly one `admin_audit_log` row).
    - E2E: `tests/e2e/test_admin_order_lifecycle.py` (5 — super_admin walks order pending→delivered over HTTP; cancel from pending; list with filters; content_editor 403 on listing; pharmacist refund 403); `tests/e2e/test_admin_reports.py` (4 — sales JSON; sales CSV with BOM byte; top-products; audit viewer).
- 428 tests pass; mypy --strict + ruff clean across 98 source files.
