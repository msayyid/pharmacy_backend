...
- **Active phase:** Phase 12 — Hardening & Launch Readiness _(complete; project at MVP-ready / v1.0.0-rc1)_
- **Status:** **complete** — 471 tests pass; coverage 80% (target 85% — 5pp gap surfaced in LAUNCH_CHECKLIST); mypy --strict + ruff clean across 119 source files.
- **Last session:** 2026-05-03
- **Sub-phases done:** 12.0 plan + 12.1 (3 parallel audit sub-agents: coverage gaps / index audit / OWASP — produced concrete punchlists), 12.2 (`/health/ready` (DB+Redis ping, 503 with structured body), `/metrics` (bearer-token via `settings.metrics_token`, module-local `CollectorRegistry`), `MetricsMiddleware` recording HTTP counter+histogram per templated route — `/metrics` + `/health` excluded from instrumentation to avoid recursion + noise; new `prometheus-client>=0.21,<1.0` dep), 12.3 (`SecurityHeadersMiddleware` injecting HSTS (HTTPS-only) + XFO DENY + nosniff + Referrer-Policy + strict CSP `default-src 'none'; frame-ancestors 'none'`; Sentry release tag is `pharmacy-api@<version>+<git_sha>` from `settings.git_sha` env var; `traces_sample_rate=0.1` + `profiles_sample_rate=0.1`), 12.4 (image-upload route inline ≤2MB / enqueue `process_image_upload` >2MB; CSV import inline ≤500 rows / enqueue `process_product_import` >500 rows + new `GET /products/imports/{id}` polling endpoint reading the Phase 11 Redis hash; `get_arq_pool` switched to lazy-init so tests via ASGITransport (which doesn't fire lifespan) can still drive routes that depend on the pool; 3 new e2e tests `test_import_async_flow.py`), 12.5 (`docs/ARCHITECTURE.md` + `docs/CONTRIBUTING.md` + 4 runbooks `docs/runbooks/{deploy,rollback,backups,incidents}.md` + `LAUNCH_CHECKLIST.md` mirroring DoD), 12.6 (`docker-compose.production.yml` with pinned `IMAGE_TAG`, healthchecks on api+worker, restart=unless-stopped; `bin/backup_db.sh` mysqldump-pipe-gzip with R2 upload commented pending Q15; `Dockerfile` adds `org.opencontainers.image.revision` label from `GIT_SHA` build arg + sets `GIT_SHA` env so Sentry release tag matches the built image; `make security-audit` (`pip-audit`) + `make backup-local` targets), 12.7 (sub-agent fixes applied: `OrderRepository.list_for_user_paginated` adds `selectinload(Order.items, Order.history)` to fix N+1 on customer order history; new migration `22f5c07c42b5` adds `idx_orders_recipient_phone` + `idx_payments_provider_txn`; bumped `python-jose>=3.5` to clear CVE-2024-49638; OWASP "P0 GUID byte-swap" finding documented as false positive — Phase 2 has 10 round-trip tests with byte-order assertions), 12.8 (BUILD_PROGRESS / CHANGELOG / DECISION_LOG / OPEN_QUESTIONS final updates; v1.0.0-rc1 release commit + tag + push).
- **Next session should:** see LAUNCH_CHECKLIST for the production-blocker list. The architecture is fully exercised through the fakes; opening the path to production traffic requires (in order): (1) Q13/Q14/Q15 vendor-doc verification + real-adapter implementation, (2) staging environment + live load-test baseline, (3) live R2 backup→restore drill. None of these are in the implementation phase plan — they're the operational handoff to the deploy team.
...
- [x] Phase 12 — Hardening & Launch Readiness _(done 2026-05-03 — v1.0.0-rc1; production blockers Q13/Q14/Q15 surfaced in LAUNCH_CHECKLIST)_
